...
if (profileKey && !profile) {
    console.error(`cloudshops-mcp-analytics: unknown CLOUDSHOPS_PROFILE='${profileKey}'. ` +
...
const missing = [];
if (!API_URL)
    missing.push("CLOUDSHOPS_API_URL");
if (!API_TOKEN)
    missing.push("CLOUDSHOPS_API_TOKEN");
if (!TENANT_ID)
    missing.push("CLOUDSHOPS_TENANT_ID");
if (missing.length > 0) {
    ...
    console.error(`cloudshops-mcp-analytics: missing required env var(s): ${missing.join(", ")}.${profileHint}`);
    process.exit(1);
}
// ---------------------------------------------------------------------------
...
const tools = [
    {
        ...
        description: "Return a markdown reference describing the analytics tables, columns, PII annotations, " +
    ...
    {
        name: "run_sql",
        description: "Execute a single SELECT (or WITH ... SELECT) query against the analytics database. " +
    ...
    {
        name: "save_analysis",
        description: "Save an analysis (text/markdown/JSON/CSV) to the tenant's S3 store under " +
    ...
    },
    {
        ...
        description: "List analyses saved by this tenant. Returns filenames, sizes, and last-modified " +
    ...
    {
        name: "load_analysis",
        description: "Load a previously saved analysis by filename. Returns the file contents as text " +
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    if (name === "save_analysis") {
        ...
        if (!filename)
            return errorResult("Error: filename is required.");
...
function formatResult(r) {
    const meta = `${r.rowCount} row(s)  |  ${r.durationMs} ms` +
        (r.truncated ? "  |  TRUNCATED at row cap (10,000)" : "");
    if (r.rowCount === 0) {
        return `${meta}\n(no rows)`;
    }
    // Compact ASCII table. Column widths sized to the longest cell or header.
    const widths = r.columns.map((c, i) => Math.max(c.length, ...r.rows.map((row) => stringify(row[i]).length)));
...
async function listSavedAnalyses(prefix) {
    ...
    if (body.count === 0)
        return ok("(no saved analyses)");
    const lines = body.objects.map((o) => `  ${o.key.padEnd(48)}  ${String(o.size).padStart(10)} bytes  ${o.lastModified ?? "-"}`);
